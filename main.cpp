...
int main(int argc, char* argv[])
{
	...
	// Example: Show the first frame of each
	if (!img1.empty()) {
		imshow("First Frame Video 1", img1[0]);
	}
	if (!img2.empty()) {
		imshow("First Frame Video 2", img2[0]);
	}
	...
	for(int k = 0; k < num_mat; k++){
		...
		*/
		...
		t = getTickCount();
#endif
		static vector<Point> corners(num_images);
		static vector<UMat> masks_warped(num_images);
		static vector<UMat> images_warped(num_images);
		static vector<Size> sizes(num_images);
		static vector<UMat> masks(num_images);
		if(k == 0){	
