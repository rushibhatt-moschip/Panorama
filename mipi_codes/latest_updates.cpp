...
int seam_calc=0; //seam calculation flag
int crop=0; //output crop flag
...
/* Helper function to compute & round duration in seconds
 * To use this easy function
 * auto start = high_resolution_clock::now(); 
 * func1(); >the funnction of which you need to calc time delay
 * auto t1 = high_resolution_clock::now();
 * cout << "func1: " << duration_s(start, t1) << " s\n";
 */
...
// Function to find largest block possible with none fully black row or column ---
...
// Function to crop the output keeping the rows/columns with 95% valid image pixel
...
static int parseCmdArgs(int argc, char** argv)
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		{
			cout << "Output window crop is enabled\n" << endl;
			crop=1;
		}
		...
		{
			cout << "Output window resize is enabled" << endl;
			resize_win=1;
		}
	}
	return 0;
...
int main(int argc, char* argv[])
{
	...
	double work_scale = 1.0, seam_scale = 1.0, compose_scale = 1;
	...
	//Default input resolution of camera is 1920,1200
	Size target_size(640, 400);
	int count = 0;
	int mipi_init_flag = 0;
	//Using ORB finder
	...
	while(1){	
		...
		// Crop the input frame for matching points (extract the region of interest)
		if(crop_input == 1){
		...
		if(fg == 1){
			...
			if (!(*adjuster)(features, pairwise_matches, cameras))
			{
				...
				cout<< "Please stay far from camera\n" << endl;
				return -1;
			}
		...
		// Seam calcultion block can be re-called when seam_calc flag is set
		if (count==0 || seam_calc == 1)
		{
			compensator->feed(corners, images_warped, masks_warped);
			seam_finder->find(images_warped_f, corners, masks_warped);
			//Mask dialation-Reduces black patches that comes when not properly stitched
			for (auto &mask : masks_warped) {
				cv::dilate(mask, mask, cv::Mat(), cv::Point(-1, -1), 2);
			}
			//	cv::imshow("img_warped_after-seam_calc",images_warped[0]);
			//	cv::waitKey();	
			//	cv::imshow("right_mask_warped_after-seam_calc",masks_warped[0]); 
			//	cv::imshow("left_mask_warped_after-seam_calc",masks_warped[1]); 
			//	cv::waitKey();	
		...
		for (int img_idx = 0; img_idx < num_images; ++img_idx)
		{
			...
			//roi = warper->warp(img, K, cameras[img_idx].R,INTER_LINEAR, BORDER_REFLECT, img_warped);
			if (count==0)
			...
			if (count==0)
				mask_roi = warper->buildMaps(img_size, K, cameras[img_idx].R, xmap1, ymap1);
			...
			if (img_warped_s.empty() || mask_warped.empty()) {
		...
		// Save the first frame result as image
		if (count==0)
			cv::imwrite(result_name,result_8u);

		//frame count
		count++;

		// Reset the seam and re-calculate
		if (key == 's' || key == 'S')
			seam_calc=1;

		// Reset everything and re-calculate new homography
		if (key == 'n' || key == 'N'){
			count=0;
			fg=1;
			seam_calc=1;
